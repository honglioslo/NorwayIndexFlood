...
DataFrame <- as.data.frame(CatShp);
Temp <- unlist(strsplit(as.character(DataFrame$stID), "[.]"));
Temp <- matrix(unlist(Temp), ncol = 3, byrow = TRUE);
CatCom <- as.numeric(Temp[,1]) * 100000 + as.numeric(Temp[,2]);

DataFrame <- as.data.frame(StaShp);
Temp <- unlist(strsplit(as.character(DataFrame$stID), "[.]"));
Temp <- matrix(unlist(Temp), ncol = 3, byrow = TRUE);
StaCom <- as.numeric(Temp[,1]) * 100000 + as.numeric(Temp[,2]);

Sta_Cat <- read.table(sprintf("%sFeltnr_flomkart_til_feltnr_GIS.txt", GISPath), sep = ";", header = FALSE);
colnames(Sta_Cat) <- c("Sta", "Cat");

for (iC in seq(1, dim(Sta_Cat)[2])) {
  CatCom[which(CatCom) == Sta_Cat[iC, 2]] <- Sta_Cat[iC, 2];
}

CatShp <- cbind(CatShp, CatCom);
StaShp <- cbind(StaShp, StaCom);
###############################################
# should match based on CatCom and StaCom
