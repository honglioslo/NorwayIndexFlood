...
Future coding:
stationary (trend) test, interdependence test

how to count non-stationarity if it exists
...
?delete, Survival Analysis for GLM, does not work
```{r, echo=FALSE}
#install.packages("survival");
library(survival)
######################################################################
## http://www.statmethods.net/advstats/glm.html do not understand the following example
help(lung)
...
#under the normal distribution, most of the Huber ρ function will fall under the |x|<k situation, which is equivalent to least squares.
...
?3.1 find homegenous regions and test hetergeniety and discortancy, working on

divide regions according to the dailt discharge rather than bare annual maximun
cluster to define homengeous region, using k-means or cluster analysis
similarity among QMED of stations as a function of something (like semi-variance graph in kriging interpolation)

remove discordant and examine hetergenity, will use dist and par later, so maybe fuzzy method about dist
